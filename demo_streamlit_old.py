...
st.markdown("<h1 style='text-align: center; color: red;'>Système de Recommandation Musical</h1>", unsafe_allow_html=True)
st.subheader("Cette application propose des **_musiques positives_** en fonction des 3 dernières écoutes d'une playlist")
...
st.write('Saisissez vos 3 dernières écoutes')
firstsong = st.text_input('Première Chanson', value="Together")
firstartist = st.selectbox('Artiste', df[df.name == firstsong].artists.unique(), key=3)
secondsong = st.text_input('Deuxième Chanson')
secondartist = st.selectbox('Artiste', df[df.name == secondsong].artists.unique(),key=1)
thirdsong = st.text_input('Troisième Chanson')
thirdartist = st.selectbox('Artiste', df[df.name == thirdsong].artists.unique(), key=2)
